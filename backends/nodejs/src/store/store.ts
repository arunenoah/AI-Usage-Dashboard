...
function tokenCost(u: { input_tokens?: number; output_tokens?: number; cache_read_input_tokens?: number; cache_creation_input_tokens?: number }): number {
  return (
    ((u.input_tokens || 0) / 1e6) * PRICE_INPUT_PER_M +
...
function isSystemInjection(text: string): boolean {
  ...
  if (noiseExact.includes(trimmed)) return true;
  ...
  if (xmlCommandTags.some(tag => trimmed.includes(tag))) return true;

  return false;
}

// ── isSpecificPrompt — mirrors Go handlers.go ────────────────────────────────
function isSpecificPrompt(text: string): boolean {
  ...
  if (codeKw.some(k => lower.includes(k))) return true;
  return false;
}

// ── scorePrompt — CARE framework, mirrors Go handlers.go ─────────────────────
function scorePrompt(pair: { user_text: string; assist_text?: string; tool_calls?: string[] }): [number, string[]] {
  const text = pair.user_text.trim();
  const tips: string[] = [];

  if (!text) return [1, ['Empty prompt — describe what you want done.']];
  ...
  for (const kw of ['as a ', 'you are ', 'act as ', 'role:', 'persona', 'background:', 'context:']) {
    if (lower.includes(kw)) { hasRole = true; break; }
  }
  if (hasRole) {
    score += 1;
  ...
  if (hasStructure && textLen >= 100) score += 1;
  ...
  for (const kw of ["don't", 'do not', 'avoid', 'must not', 'without', 'ensure', 'make sure', 'never', 'always', 'constraint', 'requirement', 'rule:', 'important:', 'must be', 'must have']) {
    if (lower.includes(kw)) { hasConstraints = true; break; }
  }
  if (hasConstraints) {
    score += 1;
  } else {
  ...
  for (const kw of ['expect', 'should return', 'should output', 'result should', 'success criteria', 'acceptance criteria', 'expected output', 'expected result']) {
    if (lower.includes(kw)) { hasExpected = true; break; }
  }
  if (hasExpected) score += 1;
  ...
  for (const kw of ['format:', 'example:', 'e.g.', 'for example', 'like this', 'such as', 'table', 'json', 'csv', 'markdown', 'return as', 'respond with']) {
    if (lower.includes(kw)) { hasFormat = true; break; }
  }
  if (hasFormat) {
    score += 1;
  } else {
  ...
  // ── CARE structural cap ───────────────────────────────────────────────────
  let careCount = 0;
  if (hasContext || hasRole) careCount++;
  if (hasAction) careCount++;
  if (hasConstraints || hasExpected) careCount++;
  if (hasFormat || hasCodeExample) careCount++;
  if (careCount < 3 && score > 7) score = 7;
  ...
  if (tips.length > 4) tips.splice(4);
...
export class Store {
  ...
  // ── statsForSourceDays ─────────────────────────────────────────────────────
  statsForSourceDays(source: string, days: number): any {
    const now = new Date();
    const from = new Date(now);
    from.setHours(0, 0, 0, 0);
    from.setDate(from.getDate() - (days - 1));
  ...
  statsForSourceRange(source: string, from: Date, to: Date): any {
  ...
  private _computeStats(allSessions: Session[], from: Date | null, to: Date | null): any {
    ...
    for (const sess of sessions) {
      ...
      if (sess.project_dir) projectSet.add(sess.project_dir);

      const day = new Date(sess.start_time).toISOString().split('T')[0];
      if (day === '0001-01-01') continue;
      if (!dailyMap[day]) {
      ...
      (d as any).cache_read += sess.total_usage?.cache_read_input_tokens || 0;
      (d as any).cache_creation += sess.total_usage?.cache_creation_input_tokens || 0;
      d.sessions++;
  ...
  // ── getConversations ───────────────────────────────────────────────────────
  async getConversations(page: number, limit: number, period: string, scoreMin: number, scoreMax: number): Promise<any> {
  ...
  private async extractConversationPairsFromFiles(): Promise<void> {
    ...
    for (const session of this.sessionList) {
      if (session.source !== 'claude-code' || !(session as any).file_path) continue;

      try {
        const turns: any[] = await (adapter as any).parseTurnsFull((session as any).file_path);

        for (let i = 0; i < turns.length; i++) {
          if (turns[i].role !== 'user' || !turns[i].text) continue;

          const userTurn = turns[i];

          // Skip system injections
          if (isSystemInjection(userTurn.text)) continue;
          ...
          for (let j = i + 1; j < turns.length; j++) {
            ...
            if (turns[j].role === 'assistant') {
              ...
              if (t.duration_ms && t.duration_ms > maxDurationMs) maxDurationMs = t.duration_ms;
  ...
  insights(): InsightsResponse {
    ...
    let tier = 'Beginner';
    if (avgPromptScore >= 9) tier = 'Expert';
    else if (avgPromptScore >= 7) tier = 'Advanced';
    else if (avgPromptScore >= 5) tier = 'Intermediate';
    ...
    const totalInput = stats.total_input_tokens as number;
    const totalOutput = stats.total_output_tokens as number;
  ...
  filterByQuery(days: number, from?: Date, to?: Date): any[] {
    const all = this.sessionList;
    if (!from && !to && days <= 0) return all;
    const cutoff = (from || (days > 0 ? new Date(Date.now() - days * 86400_000) : null));
    return all.filter(s => {
      const t = new Date(s.start_time);
  ...
  statsByProject(days: number, from?: Date, to?: Date): any {
    const sessions = this.filterByQuery(days, from, to);
    const map = new Map<string, { project: string; sessions: number; input_tokens: number; output_tokens: number; cost_usd: number }>();
  ...
  statsByModel(days: number, from?: Date, to?: Date): any {
    const sessions = this.filterByQuery(days, from, to);
    const map = new Map<string, { model: string; sessions: number; input_tokens: number; output_tokens: number; cost_usd: number }>();
    for (const s of sessions) {
  ...
  private classifyActivity(prompt: string): string {
    const p = (prompt || '').toLowerCase();
    const has = (kws: string[]) => kws.some(k => p.includes(k));
    if (has(['debug', 'fix', 'error', 'bug', 'crash', 'fail', 'broken', 'issue', 'exception', 'traceback', 'panic'])) return 'Debugging';
    if (has(['refactor', 'clean', 'simplify', 'reorganise', 'reorganize', 'restructure', 'rename', 'move', 'extract'])) return 'Refactoring';
    if (has(['add feature', 'implement', 'build', 'create', 'new endpoint', 'new component', 'new page', 'new route', 'add support', 'add the ability'])) return 'Feature Dev';
    if (has(['subagent', 'agent', 'delegate', 'spawn', 'orchestrat', 'pipeline', 'workflow'])) return 'Delegation';
    if (has(['write test', 'add test', 'unit test', 'integration test', 'spec', 'coverage', 'jest', 'pytest', 'phpunit'])) return 'Testing';
    if (has(['write code', 'implement', 'function', 'class', 'struct', 'method', 'api', 'endpoint', 'sql', 'query', 'migration', 'schema'])) return 'Coding';
    if (has(['explain', 'what is', 'how does', 'describe', 'understand', 'explore', 'analyse', 'analyze', 'review', 'read', 'look at'])) return 'Exploration';
    return 'Other';
  }

  /** Session count breakdown by classified work type. */
  statsByActivity(days: number, from?: Date, to?: Date): any {
    const sessions = this.filterByQuery(days, from, to);
  ...
  shellCommands(days: number, from?: Date, to?: Date): any {
    const sessions = this.filterByQuery(days, from, to);
    const cmdCounts: Record<string, number> = {};
    for (const s of sessions) {
      const samples: string[] = (s.tool_samples as any)?.Bash || [];
      for (const sample of samples) {
        const trimmed = sample.trim();
        if (!trimmed) continue;
        const parts = trimmed.split(/\s+/);
        let cmd = parts[0];
        if (cmd === 'sudo' && parts[1]) cmd = parts[1];
        if (cmd === '$' && parts[1]) cmd = parts[1];
  ...
  mcpServers(days: number, from?: Date, to?: Date): any {
    const sessions = this.filterByQuery(days, from, to);
    ...
    for (const s of sessions) {
      for (const [tool, count] of Object.entries(s.tool_counts || {})) {
        if (tool.startsWith('mcp__')) {
          ...
          if (parts.length >= 2) {
            serverCounts[parts[1]] = (serverCounts[parts[1]] || 0) + (count as number);
          }
        }
      }
    }
  ...
  // ── parseTurns ─────────────────────────────────────────────────────────────
  async parseTurns(sessionId: string): Promise<any[]> {
    const session = this.sessionList.find(s => s.id === sessionId);
    if (!session) return [];

    let adapter: any;
    switch (session.source) {
      case 'claude-code':   adapter = new ClaudeCodeAdapter(); break;
      case 'github-copilot': adapter = new CopilotAdapter();  break;
      case 'opencode':      adapter = new OpenCodeAdapter();   break;
      case 'windsurf':      adapter = new WindsurfAdapter();   break;
      default: return [];
    }

    try {
      const filePath = (session as any).file_path;
      if (!filePath) return [];
