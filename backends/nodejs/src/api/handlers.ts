...
export class Handler {
  ...
  private getSystemInfo(req: Request, res: Response): void {
    ...
    try {
      ...
      planCount = entries.filter((e: string) => e.endsWith('.md')).length;
    } catch (err) {
      // Directory may not exist
    }
    ...
    try {
      ...
      taskCount = entries.filter((e: any) => e.isDirectory()).length;
    } catch (err) {
      // Directory may not exist
    }
      ...
      if (cache.modelUsage) {
        ...
        info.model_usage = Object.entries(cache.modelUsage)
          .map(([model, usage]: [string, any]) => {
            const cost =
              (usage.inputTokens / 1e6) * priceInput +
          ...
          .sort((a: any, b: any) => b.est_cost_usd - a.est_cost_usd);
      }
    } catch (err) {
      info.model_usage = [];
    }
    ...
    try {
      ...
      pasteCacheCount = entries.filter((e: string) => !e.startsWith('.')).length;
    } catch (err) {
      // Directory may not exist
    }
    ...
    const fileHistSet = new Set<string>();
    try {
      const walkFileHist = (dir: string) => {
        const entries = fs.readdirSync(dir, { withFileTypes: true });
        for (const entry of entries) {
          const fullPath = path.join(dir, entry.name);
